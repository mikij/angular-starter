...
@Component({
  ...
  template: `
    <div [class]="classes()" #calendarContainer>
      ...
      <div [class]="navClasses()">
        ...
        <div class="flex items-center space-x-2">
          ...
          <z-select
            [zSize]="selectSize()"
            class="min-w-[80px]"
            [zValue]="currentYearValue()"
            [zLabel]="currentYearValue()"
            (zSelectionChange)="onYearChange($event)">
            @for (year of availableYears(); track year) {
              <z-select-item [zValue]="year.toString()">{{
                year
              }}</z-select-item>
            }
          </z-select>
      ...
      <!-- Calendar Days Grid -->
      <div class="mt-2 grid grid-cols-7 gap-0" role="rowgroup">
...
export class ZardCalendarComponent {
  ...
  protected readonly currentMonthName = computed(() => {
    ...
    if (!isNaN(selectedMonth) && this.months[selectedMonth])
  ...
  protected readonly calendarDays = computed(() => {
    ...
    const selectedDate = isNaN(navigationDate.getTime())
      ? currentDate
      : navigationDate;
    ...
    while (currentWeekDate <= endDate) {
      ...
      const isSelected = currentDate
        ? this.isSameDay(date, currentDate)
        : false;
      const isDisabled =
        this.disabled() || this.isDateDisabled(date, minDate, maxDate);
  ...
  selectDate(date: Date, i?: number) {
    ...
    this.focusedDayIndex.set(
      i ??
        this.calendarDays().findIndex((day) => this.isSameDay(day.date, date))
    );
  }
  ...
  private navigate(
    ...
    if (step === -1) {
      ...
      this.previousMonth();
      setTimeout(
        () => this.resetFocusAfterNavigation('lastWeek', dayOfWeek),
        0
      );
    } else if (step === 7) {
      ...
      this.nextMonth();
      setTimeout(
        () => this.resetFocusAfterNavigation('firstWeek', dayOfWeek),
        0
      );
    }
