...
export class ZardMenuDirective implements OnInit, OnDestroy {
  ...
  private static readonly MENU_CONTENT_SELECTOR =
    '.cdk-overlay-pane [z-menu-content]';
  ...
  private setupMenuContentListeners(): void {
    const overlay = document.querySelector(
      ZardMenuDirective.MENU_OVERLAY_SELECTOR
    );
    if (!overlay) return;

    this.addEventListenerWithCleanup(overlay, 'mouseenter', () =>
      this.cancelScheduledClose()
    );
